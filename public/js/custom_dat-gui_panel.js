...
(function () {
  ...
  _this.init = function (setupDefaultItems, params) {
    ...
    if (setupDefaultItems) {
      this.layersFolder = panel.addFolder('Стадии');
  ...
  _this.initLayersFolder = function (scene) {
    ...
    for (var layerId in mapLayers) {
      ...
      folder.add(params.lyr[layerId], 'v').name('Видимость').onChange(visibleChanged);
      folder.add(params.lyr[layerId], 'o').min(0).max(1).name('Непрозрачность').onChange(opacityChanged);
